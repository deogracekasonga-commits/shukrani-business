...
const [publishedPost] = await query(
...
const product = publishedPost || (await query('SELECT * FROM products LIMIT 1'))[0];
if (!product) {
...
if (res.ok) {
  ...
  console.log('Vérifie la table `sales` (ou le dashboard).');
}
